...
function filecache($disktag)
{
    ...
    $tag = $_SERVER['HTTP_HOST'] . '/OneSM/' . $disktag;
            ...
            function getStatus() {
                ...
                xhr.onload = function(e) {
                    if (xhr.status==200) {
                        ...
                        errordiv.innerHTML = "' . geti18n('Wait') . '" + x + "<br>" + min;
                        setTimeout(function() { getStatus() }, 1000);
                    } else {
...
function EnvOpt($needUpdate = 0)
{
    ...
    $html .= '
<a id="back" href="./">' . geti18n('Back') . '</a><br>
    <script>
...
<table border=1 width=100%>
    ...
    foreach ($EnvConfigs as $key => $val) if (isCommonEnv($key) && isShowedEnv($key)) {
        ...
        <tr>
            <td><label>' . $key . '</label></td>
            <td width=100%><input type="text" name="' . $key .'" value="' . $domain_path . '" placeholder="' . geti18n('EnvironmentsDescription')[$key] . '" style="width:100%"></td>
        </tr>';
        }*/ else $frame .= '
                <input type="text" name="' . $key . '" value="' . htmlspecialchars(getConfig($key)) . '" placeholder="' . geti18n('EnvironmentsDescription')[$key] . '" style="width:100%">';
        $frame .= '
            </td>
        </tr>';
    }
    $frame .= '
        <tr><td><input type="submit" name="submit1" value="' . geti18n('Setup') . '"></td><td></td></tr>
    </form>
</table><br>';
    ...
    <tr>
        <td>
            <form action="" method="post" style="margin: 0" onsubmit="return renametag(this);">
                ...
                <input name="_admin" type="hidden" value="">
                <input type="text" name="disktag_newname" value="' . $disktag . '" placeholder="' . geti18n('EnvironmentsDescription')['disktag'] . '">
                <input type="submit" name="submit1" value="' . geti18n('RenameDisk') . '">
            </form>
        </td>
    </tr>
</table><br>
...
<tr>
    <td>
        <form action="" method="post" style="margin: 0" onsubmit="return deldiskconfirm(this);">
            ...
            <input name="_admin" type="hidden" value="">
            <input type="submit" name="submit1" value="' . geti18n('DelDisk') . '">
        </form>
    </td>
    <td>
        <form action="" method="post" style="margin: 0" onsubmit="return cpdiskconfirm(this);">
            ...
            <input name="_admin" type="hidden" value="">
            <input type="submit" name="submit1" value="' . geti18n('CopyDisk') . '">
        </form>
    </td>
</tr>
</table>
...
<form name="' . $disktag . '" action="" method="post">
    ...
    <tr>
        <td><label>' . $key . '</label></td>
        <td width=100%><input type="text" name="' . $key . '" value="' . getConfig($key, $disktag) . '" placeholder="' . geti18n('EnvironmentsDescription')[$key] . '" style="width:100%"></td>
    </tr>';
            }
            $frame .= '
    <tr><td></td><td><input type="submit" name="submit1" value="' . geti18n('Setup') . '"></td></tr>
</form>';
        } else {
            $frame .= '
<tr>
...
<table border=1>
    ...
    <tr><td colspan="' . $num . '">' . geti18n('DragSort') . '<input type="submit" name="submit1" value="' . geti18n('SubmitSortdisks') . '"></td></tr>
    </form>
</table>
<script>
...
<table>
    <form id="change_pass" name="change_pass" action="" method="POST" onsubmit="return changePassword(this);">
        <input name="_admin" type="hidden" value="">
    <tr>
        <td>' . geti18n('OldPassword') . ':</td><td><input type="password" name="oldPass">
        <input type="hidden" name="timestamp"></td>
    </tr>
    <tr>
        <td>' . geti18n('NewPassword') . ':</td><td><input type="password" name="newPass1"></td>
    </tr>
    <tr>
        <td>' . geti18n('ReInput') . ':</td><td><input type="password" name="newPass2"></td>
    </tr>
    <tr>
        <td></td><td><button name="changePass" value="changePass">' . geti18n('ChangAdminPassword') . '</button></td>
    </tr>
    </form>
</table><br>
<table>
    ...
    <tr>
        <td>' . geti18n('AdminPassword') . ':<input type="password" name="pass">
...
function render_list($path = '', $files = [])
{
    ...
    if (substr($theme,-4)=='.php') {
        ...
        if (getConfig('adminloginpage') == null) {
            while (strpos($html, '#!LoginPage!#')) $html = str_replace('#!LoginPage!#', 'login', $html);
        } else {
            while (strpos($html, '#!LoginPage!#')) $html = str_replace('#!LoginPage!#', getConfig('adminloginpage'), $html);
        }
